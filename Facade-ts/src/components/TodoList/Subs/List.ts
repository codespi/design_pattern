...
class List extends Component {
    ...
    private static todoData: ITodoData[];//声明一个静态变量 每次实例化都是唯一的
    ...
    public static addItem(val: string) {
        ...
        if(List.todoData.length === 1) {//第一项添加要先清空
            oTodoList.innerHTML = '';
        }
    ...
    private handleListClick(e: Event) {
        const tar = e.target as HTMLElement;//类型断言  事件源对象
        const tagNme = tar.tagName.toLowerCase();//event下面没有tagname这个属性需要用到类型断言
        const oTodoItems: HTMLCollection = document.getElementsByClassName('todo-item');

        if(tagNme == 'input' || tagNme == 'button') {
